...
import ast, sys, difflib, csv, re, signal
sys.settrace
...
def jd_tfidf(doc):
	vec_bow = dictionary.doc2bow(cleanUp(doc))
...
def jd_lsi(doc):
	...
	vec_bow = dictionary.doc2bow(cleanUp(string_out))
	string_out = ''
	vec_lsi = lsi[vec_bow]
...
if arg=='new':
	lsi2 = models.LsiModel(corpus_tfidf2, id2word=dictionary2, num_topics=400)
...
def title_tfidf(doc2):
	vec_bow2 = dictionary2.doc2bow(cleanUp(doc2))
